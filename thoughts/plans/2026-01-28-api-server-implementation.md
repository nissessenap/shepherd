...
**Security Note**: The `callbackURL` originates from the adapter's `CreateTaskRequest` and is stored in the CRD's `CallbackSpec.URL` (which has kubebuilder validation: `Pattern=^https?://`). While the API is an internal service and callbacks come from trusted adapters, consider adding a callback URL allowlist (configurable via environment variable or ConfigMap) as defense-in-depth against SSRF if the API is ever exposed more broadly.
...
// Security Note: The runner status endpoint (/tasks/{id}/status) currently accepts unauthenticated
// requests. For production use, implement per-task bearer tokens (generated by the operator and
// passed to the runner pod as an environment variable) so only the legitimate runner for a given
// task can update its status. Network policies should also restrict access to this endpoint to
// runner pods only.
```
