...
#### 5. Wait for agent-sandbox in BeforeAll
**File**: `test/e2e/e2e_test.go`
...
#### 1. Add port-forward helper to test utils
**File**: `test/utils/utils.go`
**Changes**: Add a helper that starts `kubectl port-forward` in the background and returns a cleanup function. This allows the test process (running outside the cluster) to reach the in-cluster API service.

```go
// PortForward starts kubectl port-forward in the background and returns
// the local address (e.g. "localhost:18080") and a cancel function.
// It waits until the port is accepting connections before returning.
func PortForward(namespace, service string, localPort, remotePort int) (string, context.CancelFunc, error) {
	...
	cancel()
	return "", nil, fmt.Errorf("port-forward to %s/%s:%d not ready after 30s", namespace, service, remotePort)
}
```

#### 2. Add lifecycle test using the public API
**File**: `test/e2e/e2e_test.go`
**Changes**: Add a new `Describe` block that creates a task via `POST /api/v1/tasks` through a port-forward, then uses `Eventually` with kubectl to wait for each lifecycle stage. The task name comes from the API response (randomly generated by the server as `task-<8-char-random>`).
...
var _ = Describe("AgentTask Lifecycle", Ordered, func() {
	var (
		taskName      string // Set after API creates the task
		cancelPortFwd context.CancelFunc
		apiAddr       string
	)

	BeforeAll(func() {
		By("setting up port-forward to the API service")
		var err error
		// Use port 18080 to avoid conflicts. The service name includes the
		// kustomize namePrefix "shepherd-" applied to the base name "shepherd-api".
		apiAddr, cancelPortFwd, err = utils.PortForward(namespace,
			"shepherd-shepherd-api", 18080, 8080)
		...
		resp, err := http.Post(
			fmt.Sprintf("http://%s/api/v1/tasks", apiAddr),
			"application/json",
...
#### 2. Add runner logs to diagnostic collection
**File**: `test/e2e/e2e_test.go`
