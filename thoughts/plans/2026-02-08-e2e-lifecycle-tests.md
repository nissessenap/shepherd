...
### Overview
Add the core lifecycle test: create an AgentTask, wait for it to reach `Succeeded` state via the full provisioning flow (controller -> SandboxClaim -> agent-sandbox -> Sandbox -> runner -> API -> callback -> terminal).

### Changes Required:

#### 1. Add lifecycle test
**File**: `test/e2e/e2e_test.go`
**Changes**: Add a new `Describe` block with the lifecycle test after the existing "Manager" suite. The test creates an AgentTask via `kubectl apply` and uses `Eventually` to wait for each lifecycle stage.

Add a fixture file for the test task:

**File**: `test/e2e/fixtures/agenttask-lifecycle.yaml` (new file)
```yaml
...
Add the test to `test/e2e/e2e_test.go`:

```go
var _ = Describe("AgentTask Lifecycle", Ordered, func() {
	const taskName = "e2e-lifecycle"
	const taskFixture = "test/e2e/fixtures/agenttask-lifecycle.yaml"

	BeforeAll(func() {
		By("creating the AgentTask")
		cmd := exec.Command("kubectl", "apply", "-f", taskFixture)
		_, err := utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred(), "Failed to create AgentTask")
	})

	AfterAll(func() {
		By("cleaning up the AgentTask")
		cmd := exec.Command("kubectl", "delete", "-f", taskFixture, "--ignore-not-found")
		_, _ = utils.Run(cmd)
	})
