...
**Tech Stack:** Go 1.25+, controller-runtime, ghinstallation, go-github, chi (HTTP router), ko (container builds)
...
Run: `mkdir -p cmd/shepherd pkg/shepherd pkg/api pkg/operator pkg/adapters/github pkg/types api/v1alpha1 internal/controller config/crd/bases config/rbac config/manager config/samples`
...
#### Step 5: Create shepherd.go (module orchestrator stub)
...
#### Step 6: Create .ko.yaml for ko builds
...
- Add Makefile with build, test, ko targets

Co-Authored-By: Claude Opus 4.5 <noreply@anthropic.com>"
```

#### Step 10: Create worktree branches for parallel development
...
Run: `go get sigs.k8s.io/controller-runtime@v0.19.0`
...
##### Step 2: Verify CRD YAML

Run: `head -50 config/crd/bases/shepherd.io_agenttasks.yaml`
Expected: Valid CRD YAML

##### Step 3: Commit

```bash
git add -A
git commit -m "feat: generate CRD manifests
...
#### Task 6: Create Operator Module with Controller and Tests
...
7. **ko builds** and **Helm chart** for deployment
